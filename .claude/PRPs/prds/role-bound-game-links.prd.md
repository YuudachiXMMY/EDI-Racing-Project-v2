...
| 7 | End-to-end + adversarial QA | Full professor→student flow; verify a student link cannot trigger events even via crafted URL | in-progress (**adversarial WS suite 9/9 green + boot-guard verified**; Unity scene wiring + EditMode runs + live browser E2E pending — need editor-bound checkout & live stack) | - | 3, 5, 6 | `.claude/PRPs/plans/end-to-end-adversarial-qa.plan.md` (report: `.claude/PRPs/reports/end-to-end-adversarial-qa-report.md`) |
